use ai_stock_forum::{
    ...
    skills::{
        SkillDraft, SkillProvenance, SkillResource, SkillVersion, SkillVersionRef,
        SkillsProjection,
    },
};
use uuid::Uuid;
...
fn canonical_limits_and_forbidden_controls_hold_at_boundaries() {
    let valid = SkillDraft::new(
        ...
        "d".repeat(256),
        "u".repeat(512),
        (0..8).map(|index| format!("{index:0>31}")).collect(),
        "i".repeat(4_096),
        (0..8)
            .map(|index| SkillResource {
                name: format!("{index:0>63}"),
                body: "b".repeat(2_048),
            })
            .collect(),
    );
...
fn equal_accepted_content_has_stable_digest_and_exact_reference() {
    ...
    assert_eq!(reference, SkillVersionRef {
        skill_id: id(1), skill_version_id: version_id(101), version: ObjectVersion::new(1).unwrap(),
