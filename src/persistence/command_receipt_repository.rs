...
use rusqlite::{Error as SqliteError, ErrorCode, OptionalExtension, params};
...
impl CommandReceiptRepository {
    pub fn load(
        ...
        let Some((stored_id, fingerprint, request_json, capability, policy_decision, outcome_json)) =
            row
        else {
            return Ok(None);
        };
        let stored_id =
            CommandId::from_str(&stored_id).map_err(|_| PersistenceError::InvalidEventRecord)?;
        if stored_id != command_id {
            return Err(PersistenceError::InvalidEventRecord);
        }
        let command_fingerprint =
            Sha256Digest::parse(&fingerprint).map_err(|_| PersistenceError::InvalidEventRecord)?;
        ...
        for (expected, row) in rows.enumerate() {
            ...
            event_ids.push(
                EventId::from_str(&event_id).map_err(|_| PersistenceError::InvalidEventRecord)?,
            );
        }
        Ok(Some(CommandReceiptRecord {
            command_id,
            command_fingerprint,
            request_json,
            capability,
